#!/usr/bin/env python3
"""
Robust script for building/loading a LlamaIndex VectorStoreIndex from a
multimodal document folder. Replaces the previous get_or_create_index()
with safer checks and recovery behavior.
"""
...
def main(args):
    ...
    index = get_or_create_index(index_dir=INDEX_DIR, data_dir=DATA_DIR, force_rebuild=force_flag)
    query_engine = index.as_query_engine(llm=Settings.llm)

    # ----------------------------- CHAT INTERFACE -----------------------------
    def chat():
        print("\n🎓 Tutor AI Chat (type 'exit' to quit)\n")
        while True:
            try:
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Index manager and chat for Tutor AI")
    parser.add_argument("--rebuild", action="store_true", help="Force rebuilding the index from DATA_DIR")
